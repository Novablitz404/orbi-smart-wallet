...
async function scanRange(
  ...
  const filter = {
    ...
    // Soroban RPC only supports prefix topic matching — cannot wildcard intermediate
    // positions to filter by recipient (topic[2]) without specifying sender (topic[1]).
    // Filter on topic[0]="transfer" only; recipient check is done in the loop below.
  ...
  while (true) {
    ...
    try {
      ...
      // the actual valid range; resume from the real minimum so we scan every retained ledger.
  ...
  try {
    ...
    for (const token of tokens) {
      try {
        ...
        console.error(`[eventSync] token ${token.code} failed:`, tokenErr);
      }
    }
  } catch (err) {
